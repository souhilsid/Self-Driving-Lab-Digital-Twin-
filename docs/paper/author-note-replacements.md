...
| Reviewer WebGL build | `[insert anonymous reviewer-access URL and access instructions]` |
...
Use the following text only after the WebGL package has been built and tested:

> A reviewer-accessible binary of the digital twin is available as a browser-based WebGL build at `[reviewer URL]`. No installation is required. Reviewers can open the URL in a WebGL-compatible desktop browser and use `[access code, if applicable]`. The demonstration includes the digital-twin interface and representative closed-loop workflow but does not provide unrestricted access to the proprietary CEID service or physical robotic hardware. Requests for supervised access to those components may be directed to `[contact email]`.

Do not state that the reviewer binary is available until the link has been deployed and independently tested.
